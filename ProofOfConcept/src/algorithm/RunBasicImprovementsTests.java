...
public class RunBasicImprovementsTests {
	...
	public static List<BasicImprovementRoli> makeAllRunnableClasses(List<BenchmarkBlackbox> benchmarkProblems) {
		List<BasicImprovementRoli> runnables = new ArrayList<BasicImprovementRoli>();
		int counter = 0;
		for(BenchmarkBlackbox blackbox:benchmarkProblems) {
	    	BasicImprovementRoli runnable = new BasicImprovementRoli(blackbox, counter);
	...
	public static float[] runOneBenchmarkCycle() throws InterruptedException {
    	long startTime = System.currentTimeMillis();
		List<BenchmarkBlackbox> benchmarks = makeAllBenchmarkProblems();
		List<BasicImprovementRoli> runnables = makeAllRunnableClasses(benchmarks);
